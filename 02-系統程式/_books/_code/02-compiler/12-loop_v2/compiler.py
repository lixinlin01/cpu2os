...
class TokenType(Enum):
    ...
    TK_WHILE = auto(); TK_FOR = auto(); TK_BREAK = auto(); TK_CONTINUE = auto()
...
class Parser:
    def __init__(self, lexer: Lexer):
        self.lexer = lexer
        self.quads = []      # 儲存產生的中間碼
        self.string_pool = [] # 儲存字串常數（減少重複並方便 VM 處理）
        self.loop_stack =[]  # 處理迴圈嵌套與 break/continue 的跳轉回填
    ...
    def emit(self, op: str, a1: str, a2: str, res: str) -> int:
        """發送一條四位組指令，並返回該指令在 quads 中的索引（行號）"""
        idx = len(self.quads)
    ...
    def expr_or_assign(self):
        ...
        obj = name; path =[] # path 用於紀錄存取路徑（陣列索引或屬性）
    ...
    def primary(self) -> str:
        ...
        elif self.cur.type == TokenType.TK_LBRACKET: # 陣列字面量 [1, 2, 3]
    ...
    def statement(self):
        if self.cur.type == TokenType.TK_IF:
            ...
            jmp_f_idx = self.emit("JMP_F", cond, "-", "?") # 若 false 則跳轉到 ?
            ...
            self.consume(); self.expect(TokenType.TK_LPAREN, "預期 '('")
            cond_idx = len(self.quads) # 迴圈開始點
            cond = self.expression()
            self.expect(TokenType.TK_RPAREN, "預期 ')'"); self.expect(TokenType.TK_LBRACE, "預期 '{'")
            
            jmp_f_idx = self.emit("JMP_F", cond, "-", "?") # 不滿足條件則跳出
            self.loop_stack.append({'break':[], 'continue': cond_idx})
            
            while self.cur.type not in (TokenType.TK_RBRACE, TokenType.TK_EOF): self.statement()
            self.emit("JMP", "-", "-", str(cond_idx)) # 繞回開頭
            self.expect(TokenType.TK_RBRACE, "預期 '}'")
            ...
            """FOR 迴圈編織：Init -> Cond -> Body -> Step -> JMP Cond"""
            self.consume(); self.expect(TokenType.TK_LPAREN, "預期 '('")
            ...
            if self.cur.type != TokenType.TK_SEMICOLON:
                ...
                cond = self.new_t(); self.emit("IMM", "1", "-", cond)
            
            jmp_f_idx = self.emit("JMP_F", cond, "-", "?")   # 失敗跳出
            jmp_body_idx = self.emit("JMP", "-", "-", "?") # 跨過 Step 區間先去執行 Body
            ...
            self.emit("JMP", "-", "-", str(cond_idx)) # 步進後回條件檢查
            
            self.expect(TokenType.TK_RPAREN, "預期 ')'"); self.expect(TokenType.TK_LBRACE, "預期 '{'")
            
            # 4. Body (主體)
            self.quads[jmp_body_idx].result = str(len(self.quads)) # JMP_BODY 跳到這
            self.loop_stack.append({'break':[], 'continue': step_idx})
            
            while self.cur.type not in (TokenType.TK_RBRACE, TokenType.TK_EOF): self.statement()
            self.emit("JMP", "-", "-", str(step_idx)) # Body 執行完去執行步進
            self.expect(TokenType.TK_RBRACE, "預期 '}'")
...
class Frame:
    ...
    def __init__(self, ret_pc: int = 0, ret_var: str = ""):
        self.vars = {}           # 當前 Scope 的變數
        self.ret_pc = ret_pc     # 函式結束後回到的 PC
        self.ret_var = ret_var   # 函式回傳值要存入的變數名
...
class VM:
    def __init__(self, quads: list, string_pool: list):
        ...
        self.stack = [Frame()]; self.sp = 0 # 框架堆疊
        self.print_buf =[]
    ...
    def run(self):
        pc = 0; param_stack =[] # param_stack 暫存 CALL 之前的參數
        # 建立函式名稱到入口 PC 的映射
        ...
        while pc < len(self.quads):
            q = self.quads[pc]
            try:
                if q.op == "FUNC_BEG": # 略過函式定義區塊，除非是被 CALL 進來的
