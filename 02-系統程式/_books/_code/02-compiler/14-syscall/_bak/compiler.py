...
class TokenType(Enum):
    ...
    TK_PRINT = auto(); TK_WHILE = auto(); TK_FOR = auto()
    ...
    def next_token(self):
        ...
        if self.src[self.pos].isalpha() or self.src[self.pos] == '_':
            ...
            keywords = { 
                ...
                "if": TokenType.TK_IF, "else": TokenType.TK_ELSE, "print": TokenType.TK_PRINT,
...
class Parser:
    ...
    def expr_or_assign(self):
        ...
        # 處理點運算、中括號、圓括號 (處理巢狀或連續呼叫: obj.foo().bar[0])
    ...
    # ================= 基本表達式元 (Primary) =================
    # EBNF: primary ::= num | string | id | "["[ expression ("," expression)* ] "]" 
    #                 | "{"[ (id|string) ":" expression ("," (id|string) ":" expression)* ] "}" 
    #                 | "(" expression ")"
